...
public interface RenServiceInfoRepository extends JpaRepository<RenServiceInfo, String> {
    ...
    @Query(value = "select location from ren_serviceinfo where locate('WFMSComponent_Engine_', interpreter_id) > 0 and business < ?1 order by business desc limit 1", nativeQuery = true)
    List<String> findBOEngineLocationByBusiness(double threshold);

    @Query(value = "select location from ren_serviceinfo where locate('WFMSComponent_Engine_', interpreter_id) > 0 order by tomcat_concurrency asc limit 1", nativeQuery = true)
